...
[Stage 2](../2026-01-12-qrl-stage-two/) gave QRL the ability to compile relations to MBQC patterns. We could generate measurement sequences, extract graph states, and simulate pattern execution.
...
Alice has input state |ψ⟩, and shares a Bell pair |Φ+⟩ with Bob:

```
|ψ⟩_A ⊗ |Φ+⟩_BC = (α|0⟩ + β|1⟩) ⊗ (|00⟩ + |11⟩)/√2
```

### Step 2: Alice's Bell Measurement

Alice applies CNOT(A→B) then Hadamard(A), then measures both qubits:
...
```
QRL Relations → MBQC Patterns → graphix → Perceval → Quandela Hardware
```
...
- ✅ Teleportation with perfect fidelity
- ✅ Arbitrary input states (not just basis states)
- ✅ Correct Pauli correction logic
- ✅ Clean API matching MBQC theory
...
- Implement QRL → graphix converter
...
| 0 | 2-qubit relations | ✅ |
| 1 | n-qubit relations | ✅ |
| 2 | Graph extraction + pattern generation | ✅ |
| 3 | Adaptive corrections + teleportation | ✅ |
