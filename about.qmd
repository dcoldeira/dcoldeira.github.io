...
### Dexter — local-first agentic coding CLI

A single static Go binary: agent loop, tool implementations (file read/grep/edit/exec/web search/GitHub issues), provider adapters for OpenAI-compatible local backends (llama.cpp, Ollama, LM Studio, vLLM) plus a hosted-API adapter, and a Neovim client over JSON-RPC.

### Bell — fine-tuned LLM deployment

Qwen3-8B + LoRA fine-tuned on 815 instruction pairs as a self-hosted alternative to a hosted API, with sandboxed Python execution and GDPR-compliant Docker/Caddy deployment. Ran live in production.
...
A quantum programming language where entanglement is a first-class citizen — relations-first, compiling to measurement-based quantum computing. Type system where `Switch(d)` programs are provably causally nonseparable; 931 passing tests; 13.6k lines across 29 modules; four backends (Perceval, PennyLane, graphix, Qiskit).
